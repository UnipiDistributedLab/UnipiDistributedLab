...
public class ServerController {
    ...
    public static void main(String[] args) throws IOException, InterruptedException {
        ...
        for (Integer serverId : serversIds) {
            ...
            mappedServersData.add(new ServerData(port, serverId,  NodeServer.serverIP + ":" + port));
        }
        for (Integer serverId : serversIds) {
            Runnable runnable = () -> {

                Integer port = defaultPort + serverId;
                ServerData newServerData = new ServerData(port, serverId, NodeServer.serverIP + ":" + port);
